...
export default function CreateFAQ() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await API.post("/knowledge/faqs/", {
        question: form.question,
        answer: form.answer,
        category: 1, // will improve later when categories are fetched
        tags: form.tags,
        is_published: true,
      });
  ...
  return (
    <div className="p-8 max-w-[650px] mx-auto">
      <button
        className="bg-none border-none text-blue-500 cursor-pointer text-base mb-4 p-0"
        ...
        <form onSubmit={handleSubmit}>
          ...
          <label className="block font-bold mb-1 mt-4">
            ...
            <span className="font-normal text-gray-400 text-xs">
              (comma separated, e.g. login, password, account)
            </span>
          </label>
          <input
            className="w-full p-2.5 rounded border border-gray-300 box-border text-sm"
          ...
          <button
            className="mt-6 w-full p-2.5 bg-green-500 text-white border-none rounded cursor-pointer font-bold text-base"
            type="submit"
          >
            Create FAQ
          </button>
